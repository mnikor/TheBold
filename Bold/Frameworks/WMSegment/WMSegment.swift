...
//    var selectorWidth: CGFloat = 0
    ...
    @IBInspectable
    public var bottomHeight: CGFloat = 5
    ...
    func setupSelector() {
        ...
        if selectorType == .normal {
            ...
            if bottomWidth == 0 {
                bottomWidth = selectorWidth
            }
            let calcX = (selectorWidth - bottomWidth) / 2
            selector = UIView(frame: CGRect(x: calcX, y: frame.height - bottomHeight, width: bottomWidth, height: bottomHeight))
            selector.layer.cornerRadius = selector.bounds.height / 2
        }
        ...
        for (buttonIndex, btn) in buttons.enumerated() {
            ...
            if btn == sender {
                ...
                if selectorType == .bottomBar {
                    startPosition = startPosition + (frame.width/CGFloat(buttons.count) - bottomWidth) / 2
                }
                if self.animate {
